...
test('rechecks a concurrent insert conflict and accepts only an active administrator', async () => {
  ...
  await expect(ensureLocalAdmin({ databasePool, hashPassword })).resolves.toBe('existing');
  expect(databasePool.query).toHaveBeenNthCalledWith(3, 'SELECT role, is_active FROM auth_users WHERE email = $1', ['admin@getdefy.co']);
  expect(databasePool.query).toHaveBeenCalledTimes(3);
});

test.each([[[]], [[{ is_active: false, role: 'admin' }]], [[{ is_active: true, role: 'user' }]]])('rejects a missing or conflicting administrator after a concurrent insert', async rows => {
  const { ensureLocalAdmin } = await loadAdminBootstrap();
  const databasePool = createPool();
  const hashPassword = jest.fn().mockResolvedValue('$2b$12$local-admin-hash');

  databasePool.query.mockResolvedValueOnce({ rows: [], rowCount: 0 }).mockResolvedValueOnce({ rows: [], rowCount: 0 }).mockResolvedValueOnce({ rows, rowCount: rows.length });
